...
echo "set logscale x" >> tmp.gpl
echo "set xlabel \"95%tile Delay (ms)\"" >>tmp.gpl
...
echo "set output \"95_pkt_${1}.svg\"" >> tmp.gpl
...
tmp=$1
n=${tmp%.tr.tsv}
source settings/95th-setting-$n.sh

for cnt in `seq 0 6`
do

    cmd="$cmd \"<(sed -n '$((cnt+2))p' $1)\"  using 5:3:1 ${D[${cnt}]} notitle,"
done
cmd=${cmd%,};
echo $cmd >> tmp.gpl
