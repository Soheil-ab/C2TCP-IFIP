...
echo 'set xrange [20:200] reverse' >> tmp.gpl
...
echo "set terminal svg size 300,150 dynamic enhanced fname 'arial'  fsize 8" >> tmp.gpl
#echo "set terminal epslatex color colortext standalone" >> tmp.gpl
#echo "set hidden3d" >> tmp.gpl
echo "set output \"target95.svg\"" >> tmp.gpl
...
D[1]="with labels offset 0,-1 tc rgb \"#0000FF\" point pointtype 3 ps 1 lc rgb \"#0000FF\""
D[0]="with labels offset 0,1 tc rgb \"#FF0000\" point pointtype 1 ps 1 lc rgb \"#FF0000\""


cmd="$cmd \"<(sed -n '1p' $1)\"  using 4:2:1 ${D[0]} notitle,"
cmd="$cmd \"<(sed -n '2,10p' $1)\"  using 4:2:1 ${D[1]} notitle,"
